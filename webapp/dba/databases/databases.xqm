...
function _:databases(
  ...
  tmpl:wrap(map { 'top': $_:CAT, 'info': $info, 'error': $error }, (
    <td width='600'>
      <form action="{ $_:CAT }" method="post" class="update">
        <h2>Databases</h2>
        {
          ...
          return html:table($entries, $headers, $buttons, 'database', map { }, $sort, true())
        }
      </form>
    </td>,
    <td class='vertical'/>,
    <td>
      <table>
        <tr>{
          let $table := html:properties(web:eval('db:system()'))
...
(:~
 : Performs updates on specified databases.
