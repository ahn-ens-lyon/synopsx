...
function _:rename(
  ...
  try {
    if(web:eval('db:exists($n, $m)', map { 'n' : $name, 'm': $newname })) then (
      ...
      web:eval("db:rename($n, $r, $m)", map { 'n': $name, 'r': $resource, 'm': $newname }),
      web:redirect($_:SUB, map { 'info': 'Resource was renamed.', 'name': $name, 'resource': $newname })
    )
  } catch * {
    web:redirect("rename", map { 'error': $err:description, 'name': $name, 'resource': $resource,
      'newname': $newname })
  }
};
