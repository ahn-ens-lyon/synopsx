    ...
    <td>
      <form action="replace" method="post" enctype="multipart/form-data">
        ...
        <h2>
          ...
          { web:link($resource, $_:SUB, map { 'name': $name, 'resource': $resource } ) } »
  ...
  try {
    ...
    return if($file) then (
      try {
        ...
        return web:eval("db:replace($n, $r, $i)", map { 'n': $name, 'r': $resource, 'i': $i }),
        web:redirect($_:SUB, map { 'redirect': $_:SUB, 'name': $name, 'resource': $resource,
          'info': 'Replaced resource: ' || $resource })
      } catch * {
        error($err:code, replace($err:description, '^.*\): ', ''))
      }
    ) else (
      web:redirect("replace", map { 'redirect': $_:SUB, 'name': $name, 'resource': $resource,
        'error': 'Please select a file to upload.' })
    )
  } catch * {
    web:redirect("replace", map { 'error': $err:description, 'name': $name, 'resource': $resource })
  }
};
